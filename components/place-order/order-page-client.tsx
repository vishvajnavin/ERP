...
const PlaceOrderPage = ({
    ...
    const mockProducts: { id: string; type: "Sofa" | "Bed"; name: string; price: number; details: Product }[] = useMemo(() => {
        const sofas = initialSofaModels.map(sofa => ({
            id: sofa.id,
            type: 'Sofa' as const,
            name: sofa.model_name,
            price: 0,
            details: { ...sofa },
        }));
        const beds = initialBedModels.map(bed => ({
            id: bed.id,
            type: 'Bed' as const,
            name: bed.model_name,
            price: 0,
    ...
    const addNewItem = () => {
        setOrderItems([...orderItems, { ...initialProductState, uniqueId: Date.now() }]);
    };
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <button onClick={addNewItem} className="w-full flex items-center justify-center gap-2 py-3 border-2 border-dashed border-gray-300 text-gray-500 font-semibold rounded-lg hover:bg-gray-200 hover:border-gray-400 transition">
...
const OrderSummary = ({ total, itemCount }: { total: number, itemCount: number }) => (
    <div className="bg-white p-6 rounded-xl shadow-md">
        ...
        <button className="w-full mt-6 flex items-center justify-center gap-2 py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700">
